...
class LinkyFunctionalTest extends WebDriverTestBase {
  ...
  public function testLinkyWidget() {
    ...
    $id = $match[1];
    $assert_session->elementTextContains('css', '[data-drupal-messages]', sprintf('entity_test %s has been created.', $id));
    ...
    $page->findButton('Save')->click();
    $assert_session->elementTextContains('css', '[data-drupal-messages]', 'You must provide a title.');
    ...
    $page->findButton('Save')->click();
    $assert_session->elementTextContains('css', '[data-drupal-messages]', 'You have entered an invalid URL. Please enter an external URL.');
    // Validation should fail with invalid entity.
    $page->findField('field_linky[4][target_id]')->setValue('I do not exist (123)');
    $page->findButton('Save')->click();
    $assert_session->elementTextContains('css', '[data-drupal-messages]', 'The referenced entity (linky: 123) does not exist.');

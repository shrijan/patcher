...
class DropzoneJsEbWidget extends WidgetBase {
  ...
  protected function getFiles(array $form, FormStateInterface $form_state) {
    ...
    if (\Drupal::getContainer()->has('file.validator')) {
      $additional_validators = [
        'FileSizeLimit' => [
          'fileLimit' => Bytes::toNumber($config['settings']['max_filesize']),
          0
        ]
      ];
    }
  ...
  protected function handleWidgetContext($widget_context) {
    ...
    if (\Drupal::getContainer()->has('file.validator')) {
      if (isset($validators['FileSizeLimit'])) {
        $this->configuration['max_filesize'] = $validators['FileSizeLimit']['fileLimit'];
      }
      if (isset($validators['FileExtension'])) {
        $this->configuration['extensions'] = $validators['FileExtension']['extensions'];
      }
    }
    else {
